...
# cambiamos a usuario raiz para no usar sudo
sudo -i
...
sudo bash -c "cat \"$PATH_PKI/ldapcert.pem\" > \"$PATH_PKI/cacerts.pem\""

# creamos el fichero add-tls
sudo bash -c " cat << EOL >> add-tls.ldif
dn: cn=config
changetype: modify
